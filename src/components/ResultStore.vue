<template>
	<div class="result-content-holder">
		<div id="result-details">
			<div id="store-info">
				<img id="store-icon" :src="require(`../assets/images/shop_icon/${storeDetails.icon}`)" alt="store-icon">
				<div id="store-text-holder">
					<label id="store-name" :style="`color: ${storeDetails.primaryColor};`">{{storeDetails.displayName}}
					</label>
					<label id="store-website">{{storeDetails.website}}</label>
				</div>
			</div>
			<span id="upper" class="section-separator"></span>
			<div id="store-products-details">
				...
				<label id="avg-price" class="dataText">
					{{getDataForStore(storeDetails.name).average}}
				</label>
			</div>
...
<script lang="ts">
	...
	export default class ResultStore extends Vue {
		public getDataForStore!: any;
		public gotoDetails() {
			this.$router.push({
				query: {
					...this.$route.query,
					shop: this.$props.storeDetails.name,
				},
			});
		}

		public get productsFound() {
			let dataForStore = this.getDataForStore(this.$props.storeDetails.name);
			if (dataForStore != null) {
				return dataForStore.matches;
			}
			console.log(this.$props.storeDetails.name);
			return 0;
		}
	}
</script>
